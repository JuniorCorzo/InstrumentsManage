...
export class UnitProcessService {
  private readonly logs = new Logger(UnitProcessService.name, { timestamp: true })
  ...
  public async getAllUnitProcess (): Promise<ResponseDTO<UnitProcessDTO>> {
    ...
    const data = (await this.unitProcessRepository.findAll()).map(unitProcess => UnitProcessAdapter.toDTO(unitProcess))
  ...
  public async getUnitProcessById (id: string): Promise<ResponseDTO<UnitProcessDTO>> {
    this.logs.log(`[GET] Fetch the process unit with record ${id} of MongoDB`)
    const data = UnitProcessAdapter.toDTO(await this.unitProcessRepository.findById(id))
  ...
  public async insertUnitProcess (unitProcess: UnitProcessDTO): Promise<ResponseDTO<UnitProcessDTO>> {
  ...
  public async updateUnitProcess (unitProcess: UnitProcessDTO): Promise<ResponseDTO<UnitProcessDTO>> {
  ...
  public async deleteUnitProcess (id: string): Promise<ResponseDTO<UnitProcessDTO>> {
