...
public class InMemoryUnitProcessContext implements IDataContext<UnitProcessDTO> {
    ...
    public static InMemoryUnitProcessContext getInstance(UnitProcessService unitProcessService) {
    ...
    public UnitProcessDTO get(String key) {
        ...
        try {
            if (!this.unitProcessContext.containsKey(key)) {
                lock.writeLock().lock();
                try {
                    UnitProcessDTO unitProcessById = this.unitProcessService.getUnitProcessById(key);
                    this.unitProcessContext.put(unitProcessById.id(), unitProcessById);
